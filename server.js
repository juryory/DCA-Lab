...
<body>
  ...
  <script>
    ...
    function render(){
      ...
      if(details.length){var cn=details.length,ar=details.reduce(function(s,d){return s+(Number(d.returnRate)||0)},0)/cn,ti=details.reduce(function(s,d){return s+(Number(d.totalInvested)||0)},0),tv=details.reduce(function(s,d){return s+(Number(d.finalValue)||0)},0),tt=details.reduce(function(s,d){return s+(Number(d.tradeCount)||0)},0);detail.innerHTML='<div class="mini-wrap"><table class="mini-table"><thead><tr><th>窗口</th><th>收益率</th><th>投入</th><th>最终市值</th><th>交易数</th></tr></thead><tbody>'+details.map(d=>'<tr><td>'+esc(d.range||"-")+'</td><td class="'+(Number(d.returnRate)>=0?"good":"bad")+'">'+pctScore(d.returnRate)+'</td><td>'+money(d.totalInvested)+'</td><td>'+money(d.finalValue)+'</td><td>'+esc(d.tradeCount??"-")+'</td></tr>').join("")+'</tbody><tfoot><tr style="font-weight:700;border-top:2px solid #dfd3c0"><td>平均/合计</td><td class="'+(ar>=0?"good":"bad")+'">'+pctScore(ar)+'</td><td>'+money(ti)+'</td><td>'+money(tv)+'</td><td>'+tt+'</td></tr></tfoot></table></div>'}else{detail.innerHTML=''}
