...
<body>
  ...
  <script>
    ...
    function renderAmountCard(key, label, entry) {
    ...
    function renderStrategyCard(entry) {
      const cfg = (entry.configs || {})["au9999"] || (entry.configs || {})["csi300"] || (entry.configs || {})["sp500"];
      if (!cfg) return "";
      ...
      return \`<div class="asset-card full-row">
        <div class="asset-head">
          <div class="asset-name">策略参数（三资产共享）</div>
        </div>
        <div class="asset-points" style="grid-template-columns:repeat(auto-fill,minmax(180px,1fr))">
    ...
    function renderParamsHtml(entry) {
      ...
      return \`<div class="asset-grid">
        \${renderAmountCard("au9999", "黄金 AU9999", entry)}
        \${renderAmountCard("csi300", "沪深300", entry)}
        \${renderAmountCard("sp500", "标普500", entry)}
        \${renderStrategyCard(entry)}
      </div>\`;
    }
